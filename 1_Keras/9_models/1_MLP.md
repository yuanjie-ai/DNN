...
class KerasMLP(object):
    def __init__(self, X, y, optimizer=Adam(), batch_size=32, nb_epoch=10, best_model_weight="best_model_weights.hdf5"):
        ...
        checkpointer = ModelCheckpoint(self.best_model_weight, verbose=0, save_best_only=True)
        annealer = LearningRateScheduler(
            lambda x: K.get_value(self.model.optimizer.lr) * 0.9 ** x)  # Dynamic learning rate
        self.model.fit(X, y, batch_size, nb_epoch, verbose=2, callbacks=[checkpointer, annealer], validation_split=0.2)
